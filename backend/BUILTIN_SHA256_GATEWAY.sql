-- Attempt using built-in sha256 (PostgreSQL 13+)
-- This avoids dependency on pgcrypto extension if it's being difficult.
...
DECLARE
    ...
    v_result JSONB;
BEGIN
    -- Use built-in sha256 function (requires PG 13+)
    -- We cast p_agent_key to bytea for sha256, then encode the result to hex
